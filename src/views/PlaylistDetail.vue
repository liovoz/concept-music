        ...
        <div class="flex flex-col justify-end flex-1 min-w-0">
          ...
          <div class="flex items-center space-x-3">
            ...
            <button v-if="!isCreatedByMe" @click="toggleCollect" :disabled="isCollectLoading" class="px-6 py-2.5 rounded-full text-sm font-bold transition-all transform active:scale-95 flex items-center no-drag" 
              ...
              <AppIcon v-if="isCollectLoading" name="spinner" spin class="h-5 w-5 mr-1.5" />
...
const playAll = () => {
  ...
  const fallbackCover = playlistInfo.value.cover || defaultImg;
  const initialSongs = songs.value.map(song => buildPlayPayload(song, fallbackCover));
  const result = store.startPlayAllHydrationSession(initialSongs, { sourceKey: `playlist:${route.params.id}` });
  ...
  startPlayAllHydration({
    ...
    loadPage: async (nextPage) => {
      const songsRes = await request.get('/playlist/track/all', {
        params: { id: targetFetchId, page: nextPage, timestamp: Date.now() },
        silent: true
      });
      const rawSongs = extractSongs(songsRes);
      let normalized = normalizeSongs(rawSongs, defaultImg);
      ...
      return {
        ...
        songs: normalized.map(song => buildPlayPayload(song, fallbackCover))
      };
    },
    appendSongs: (sessionId, payloads) => store.extendPlayAllHydration(sessionId, payloads),
    shouldContinue: () => String(route.params.id || '') === routeId && store.isPlayAllHydrationActive(result.sessionId)
  });
};
</script>
