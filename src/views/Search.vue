...
const playAll = () => {
  ...
  const result = store.startPlayAllHydrationSession(initialSongs, { sourceKey: `search:${keyword}` });
  if (!result.sessionId || !hasMore.value || !keyword) return;
  ...
  startPlayAllHydration({
    ...
    loadPage: async (nextPage) => {
      const res = await request.get('/search', {
        params: { keywords: keyword, type: 'song', page: nextPage, pagesize: pageSize },
        silent: true
      }).catch(() => null);
      ...
      if (rawSongs.length === 0) {
        const complexRes = await request.get('/search/complex', {
          params: { keywords: keyword, page: nextPage, pagesize: pageSize },
          silent: true
        }).catch(() => null);
        rawSongs = extractSongs(complexRes);
      }
    ...
    shouldContinue: () => currentKeyword.value === keyword && store.isPlayAllHydrationActive(result.sessionId)
  });
};
</script>
