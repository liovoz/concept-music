...
const getImportErrorMessage = (error, target) => {
  ...
  if (error?.code === 'NETEASE_PLAYLIST_NOT_FOUND') {
    return target ? `未找到对应网易云歌单（${target}），请检查后重试` : '未找到该网易云歌单，请检查后重试';
  }
  return '导入失败，未找到该网易云歌单或网络暂时异常';
};
...
const fetchPlaylist = async (target, options = {}) => {
  ...
  if (!targetId && !targetUrl) {
    const message = '缺少有效的网易云歌单 ID 或链接';
    setError(message);
  ...
  if (targetId && !forceRefresh && restoreCachedPlaylist(targetId)) {
    setLastNeteasePlaylistId(targetId);
  ...
  try {
    ...
    const detail = await request.get('/netease/playlist/detail', { params });
    ...
    setLastNeteasePlaylistId(realId);
    rememberCurrentPlaylist();
    if (isManualImport) inputValue.value = '';
    nextTick(() => setupObserver());
    router.replace({ name: 'NeteaseImportDetail', params: { id: realId } });
    if (notifySuccess) {
...
const submitImport = () => {
  ...
  // 1. 尝试直接提取 7-14 位数字 ID 或标准网易云链接中的 ID
  const directId = extractNeteasePlaylistId(raw);
  if (directId) {
    importPlaylist(directId);
    return;
  }

  // 2. 尝试识别网易云短链接（如 163cn.tv/xxx）
  const shortUrl = extractNeteaseShortUrl(raw);
  if (shortUrl) {
  ...
  // 3. 非合法格式，予以明确拦截提示
  store.showToast('请输入有效的网易云歌单链接或 ID（支持 7-14 位数字 ID 或网易云音乐歌单分享链接）');
};
