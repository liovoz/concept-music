// ====================
// 文件：src/views/RankDetail.vue
// ====================
<template>
  <div class="h-full overflow-y-auto custom-scrollbar flex flex-col relative" id="rank-detail-scroll-container">
    
    <div class="absolute top-0 left-0 w-full h-80 bg-gradient-to-b from-gray-100 to-white -z-10"></div>

    <div class="p-8 z-10 flex-1 flex flex-col">
      ...
      <div class="flex items-end mb-10">
        ...
        <div class="ml-6 flex flex-col justify-end flex-1 min-w-0">
          ...
          <div v-else class="text-xs font-bold text-blue-600 tracking-widest uppercase mb-2">Top Charts</div>

          <div v-if="isLoading && page === 1" class="w-3/4 h-10 bg-gray-200 rounded animate-pulse mb-4"></div>
          <h2 v-else class="text-4xl font-extrabold text-gray-900 tracking-tight line-clamp-2 mb-3" v-tooltip="rankInfo.name">{{ rankInfo.name || '未知榜单' }}</h2>

          <div v-if="isLoading && page === 1" class="w-1/2 h-4 bg-gray-200 rounded animate-pulse mb-6"></div>
          <p v-else class="text-sm text-gray-500 line-clamp-2 mb-6 leading-relaxed pr-10 cursor-default whitespace-pre-wrap" v-tooltip="rankInfo.intro">{{ rankInfo.intro || '暂无榜单简介...' }}</p>
      ...
      <div v-if="isLoading && page === 1" class="space-y-4">
      ...
      <div v-else-if="isError && page === 1" class="flex-1 flex flex-col items-center justify-center py-20 text-red-500 bg-red-50/50 rounded-3xl border border-red-100">
        ...
        <p class="text-base font-bold text-red-600 mb-2">获取榜单详情失败</p>
      ...
      <div v-else-if="songs.length === 0" class="flex-1 flex flex-col items-center justify-center py-32 text-gray-400">
        <svg class="w-20 h-20 mb-4 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
        <p class="text-sm font-medium tracking-widest">该排行榜暂无歌曲数据</p>
      </div>
    ...
    <BackToTop targetId="rank-detail-scroll-container" />
  </div>
</template>
...
const extractRankInfo = (res) => {
  const data = res?.data || {};
  let cover = data.img_9 || data.banner_9 || data.imgurl || '';
  if (cover) cover = cover.replace(/\{size\}/g, '400');
  let introStr = data.intro || '暂无榜单简介';
  introStr = introStr.replace(/\\r\\n/g, '\n').replace(/\r\n/g, '\n');

  return {
    name: data.rankname || '未知榜单',
    cover: cover,
    intro: introStr
  };
};

const extractSongsList = (res) => {
  if (res?.data?.songlist && Array.isArray(res.data.songlist)) {
    return res.data.songlist;
  }
  if (res?.data?.info && Array.isArray(res.data.info)) return res.data.info;
  return [];
};
...
const fetchDetail = async () => {
  ...
  try {
    ...
    const [infoRes, audioRes] = await Promise.allSettled([
      request.get('/rank/info', { params: { rankid: rankId } }),
      request.get('/rank/audio', { params: { rankid: rankId, page: page.value, pagesize: pageSize } })
    ]);

    if (infoRes.status === 'fulfilled') {
      rankInfo.value = extractRankInfo(infoRes.value);
    } else {
      throw new Error('获取榜单头部信息失败');
    }

    if (audioRes.status === 'fulfilled') {
      const rawSongs = extractSongsList(audioRes.value);
      songs.value = normalizeSongs(rawSongs, defaultImg);
      if (rawSongs.length < pageSize) hasMore.value = false;
    } else {
      throw new Error('获取榜单歌曲列表失败');
    }

  } catch (error) {
    isError.value = true;
    errorMessage.value = error.message || '网络请求失败，请检查配置。';
  } finally {
...
const loadMore = async () => {
  ...
  try {
    const audioRes = await request.get('/rank/audio', { params: { rankid: rankId, page: page.value, pagesize: pageSize } });
    const newRawSongs = extractSongsList(audioRes);
    
    if (newRawSongs.length === 0) {
      hasMore.value = false;
    } else {
      songs.value.push(...normalizeSongs(newRawSongs, defaultImg));
      if (newRawSongs.length < pageSize) {
        hasMore.value = false;
      }
    }
  } catch (error) {
...
const playAll = () => {
  ...
  const fallbackCover = rankInfo.value.cover || defaultImg;
  const initialSongs = songs.value.map(song => buildPlayPayload(song, fallbackCover));
  const result = store.startPlayAllHydrationSession(initialSongs, { sourceKey: `rank:${route.params.id}` });
  if (!result.sessionId || !hasMore.value) return;

  const rankId = String(route.params.id || '');
  startPlayAllHydration({
    sessionId: result.sessionId,
    startPage: page.value + 1,
    pageSize,
    loadPage: async (nextPage) => {
      const audioRes = await request.get('/rank/audio', {
        params: { rankid: rankId, page: nextPage, pagesize: pageSize, timestamp: Date.now() },
        silent: true
      });
      const rawSongs = extractSongsList(audioRes);
      return {
    ...
    shouldContinue: () => String(route.params.id || '') === rankId && store.isPlayAllHydrationActive(result.sessionId)
  });
};
</script>
